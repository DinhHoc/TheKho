...
<div class="search-box no-print">
    ...
    <form method="POST" action="/phieu-nhap" class="search-form">
        @csrf

        <input 
            name="so_phieu_nhap"
            class="search-input"
            placeholder="🔍 Nhập số chứng từ nhập..."
            required
        >
    ...
    <div style="display:flex; gap:10px;">
        ...
        <form method="POST" action="/phieu-nhap/store">
            @csrf
            <input type="hidden" name="so_phieu_nhap" value="{{ $data[0]->so_phieu_nhap }}">
            <input type="hidden" name="ngay_nhap" value="{{ $data[0]->ngay_nhap }}">
...
<div class="info-top">
    <div>
        ...
        <b>Kho nhập:</b> {{ $data[0]->ma_kho }}
    </div>

    <div>
        <b>Ngày nhập:</b> 
        {{ \Carbon\Carbon::parse($data[0]->ngay_nhap)->format('d/m/Y') }} <br>
        <b>Số chứng từ:</b> {{ $data[0]->so_phieu_nhap }}
    </div>
</div>
